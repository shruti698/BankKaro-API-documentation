...
SET 
  ...
  validation_notes = ARRAY['mobile must be a valid 10-digit number', 'otp must be exactly 6 digits', 'Authorization header with partner token is required', 'OTP expires after 5 minutes'],
...
SET 
  ...
  validation_notes = ARRAY['x-api-key must be a valid API key provided by BankKaro', 'This API must only be called from the server-side to prevent leaking the x-api-key', 'Do not expose this API in frontend JavaScript or browser tools', 'The jwttoken is time-bound and should be refreshed before expiry'],
...
SET 
  ...
  validation_notes = ARRAY['credit_score must be one of: 600, 650, 750, 800', 'sort_by must be one of: recommended, annual_savings, annual_fees', 'card_networks must be one or more of: VISA, Mastercard, RuPay, AmericanExpress', 'annualFees should be in format ''min-max'' (e.g., ''0-15000'')', 'free_cards should be boolean (true/false) or omitted'],
...
SET 
  ...
  validation_notes = ARRAY['partner-token header is required', 'Authorization header with user token is required'],
  field_table = '[]'::jsonb,
  sample_responses = '[]'::jsonb,
